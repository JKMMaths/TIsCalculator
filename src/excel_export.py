...
from .topological_indices import INDEX_METADATA, INDEX_ORDER
...
def create_excel_workbook(result: MoleculeAnalysisResult) -> bytes:
    ...
    indices_sheet.write_row(0, 0, ["No.", "Topological Index Name", "Symbol", "Mathematical Formula", "Calculated Value"], bold)
    for row_idx, name in enumerate(INDEX_ORDER, start=1):
        value = result.index_values.get(name)
        metadata = INDEX_METADATA[name]
        indices_sheet.write_number(row_idx, 0, row_idx)
        indices_sheet.write(row_idx, 1, metadata["name"])
        indices_sheet.write(row_idx, 2, name)
        indices_sheet.write(row_idx, 3, metadata["formula"])
        if value is None:
            indices_sheet.write(row_idx, 4, "Requires specification")
        else:
            indices_sheet.write_number(row_idx, 4, value)
    # Make the worksheet useful as a printed report as well as a data export.
    indices_sheet.set_landscape()
    indices_sheet.fit_to_pages(1, 0)
    indices_sheet.repeat_rows(0)
    indices_sheet.set_header("&C&BTOPOLOGICAL INDICES&B")
    indices_sheet.set_footer("&LGenerated by Drug Topology Calculator&CPage &P of &N")
    indices_sheet.set_column(0, 0, 6)
    indices_sheet.set_column(1, 1, 38)
    indices_sheet.set_column(2, 2, 14)
    indices_sheet.set_column(3, 3, 52)
    indices_sheet.set_column(4, 4, 22)
